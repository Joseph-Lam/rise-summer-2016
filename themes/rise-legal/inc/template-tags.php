 ...
 */
  ...
  */
 function rise_legal_comment_count() {
  ...
  */
 function rise_legal_entry_footer() {
 	// Hide category and tag text for pages.
 	if ( 'post' === get_post_type() ) {
 		/* translators: used between list items, there is a space after the comma */
 		$categories_list = get_the_category_list( esc_html( ', ' ) );
 		if ( $categories_list && rise_legal_categorized_blog() ) {
  ...
  * @return bool
  */
 function rise_legal_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'rise_legal_categories' ) ) ) {
 		...
 		set_transient( 'rise_legal_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so rise_legal_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so rise_legal_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in rise_legal_categorized_blog.
  */
 function rise_legal_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'rise_legal_categories' );
 }
 add_action( 'edit_category', 'rise_legal_category_transient_flusher' );
 add_action( 'save_post',     'rise_legal_category_transient_flusher' );
  ...
  */
 function rise_legal_comment_list( $comment, $args, $depth ) {
  ...
  */
 function rise_legal_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
